...
**Scope note:** this removes the **single-step UPDATE/PATCH vector** and reduces attack surface; it does **not** fully close the bypass. `insert_own`/`delete_own` stay open to `authenticated` and INSERT does not constrain `retention_until`, so a user can still move their window via `DELETE`+re-`INSERT`. Closing that residual vector needs DB-level immutability design and is a **deferred follow-up** — not claimed by this change.
...
The table exposes only select/insert/delete to `authenticated`. A logged-in user's **single-step** PATCH on their own `retention_until` returns 403 / permission denied with the value unchanged. The request → cancel → sweep lifecycle is unaffected. Reversible by re-creating the policy. (The `DELETE+INSERT` re-open vector remains and is a deferred follow-up — full DB-layer immutability is not claimed.)
...
- Dropping UPDATE narrows but does not eliminate the retention-window bypass: `DELETE`+re-`INSERT` via PostgREST still lets a user set an arbitrary `retention_until` (INSERT has no value constraint). Deferred follow-up; this change does not claim full DB-layer immutability.

## Success Criteria (Summary)

- A logged-in user can no longer change their own `retention_until` via the **single-step UPDATE PATCH** — the same PATCH that succeeded pre-migration is rejected (403 / permission denied) with the value unchanged. (The two-step `DELETE+INSERT` re-open path is a deferred follow-up, not closed by this change.)
