...
- [ ] 1.1 `review.astro` — wrapper, `PageHeader`, read-only `Notice`
- [ ] 1.2 `ReviewSession.tsx` — three terminal states, delete `DoneCard`
- [ ] 1.3 `ReviewSession.tsx` — session container, progress row, restart `Button`, rating-error `Notice`
- [ ] 1.4 `ReviewSession.tsx` — card face, reveal button, rating-button tiers, `Kbd`, hint row

### Phase 2: Make the acceptance criteria executable

- [ ] 2.1 Add `src/components/review/review-paper.test.ts` source-level guard
- [ ] 2.2 Full validation gate green, including `test:e2e` with both review specs unmodified
- [ ] 2.3 Manual walkthrough and screenshot evidence for the non-greppable AC clauses
