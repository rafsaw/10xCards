...
Guidance for Claude Code working in this repository. For stack details, setup, scripts, and auth routes, see `@README.md`.

## Critical rules — read first

- **Environment is Windows + PowerShell.** Use PowerShell syntax: `$env:VAR`, `Test-Path`, `;` to chain commands (never `&&`). The user's notes mix Polish and English — reply in the language of their latest message.
...
- **`context/` is the source of truth for the 10x workflow** — PRD, tech-stack, shape-notes, plans, and frames live there and are never regenerated by scaffolding. Do not overwrite them or treat them as disposable.
- **The regeneration trap.** Everything between the `<!-- BEGIN @przeprogramowani/10x-cli -->` and `<!-- END @przeprogramowani/10x-cli -->` markers below is overwritten on every `10x get`. Put persistent project guidance **above** the first BEGIN marker — edits inside the markers are lost. The same applies to `.claude/skills/10x-*/` (CLI-managed, tracked in `.claude/.10x-cli-manifest.json`).

## What this repo is

**10xCards** — an AI-assisted spaced-repetition flashcard MVP. A user pastes a passage, AI proposes candidate cards, and the user accepts/rejects each one before it enters their study library. Greenfield web app; single-user dogfood for v1, multi-user architecture from day one. PRD: `context/foundation/prd.md`. Shaping notes: `context/foundation/shape-notes.md`.

## Project state

The project is scaffolded on the **10x-astro-starter** — Astro 6 + React 19 + TypeScript + Tailwind 4 + Supabase auth + Cloudflare Workers (`@README.md` has the details). What exists today is the starter's skeleton: auth pages (`src/pages/auth/`), auth API routes (`src/pages/api/auth/`), and route protection in `src/middleware.ts`. The 10xCards product features — passage paste, AI candidate-card generation, accept/reject review — are **not built yet**.

Lesson chain: currently **m1l4** (per `.claude/.10x-cli-manifest.json`) — agent context. PRD, `context/foundation/tech-stack.md`, and bootstrap scaffolding are done. The starter's own CLAUDE.md was sidelined to `CLAUDE.md.scaffold` during bootstrap. Commits run `eslint --fix` on staged files via husky + lint-staged, so committing may modify your files.
...
- `npx @przeprogramowani/10x-cli@latest get mXlY` — pull lesson `mXlY` skills into `.claude/skills/`; the tool set lives in `C:\Users\rafal\AppData\Roaming\10x-cli\config.json` (`claude-code`). `--tool claude-code` overrides it.
...
Helper meta-skills (`10x-cli-setup`, `10x-cli-guide`) install globally to `C:\Users\rafal\.agents\skills\` via `npx skills add przeprogramowani/10x-cli -g`. With `-g` they load without appearing under `.claude/skills/`; without it they scope to this project only.
